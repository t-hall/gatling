...
case class JmsReqOnlyActionBuilder(attributes: JmsAttributes)(implicit configuration: GatlingConfiguration) extends ActionBuilder {
  ...
  def build(system: ActorSystem, ctx: ScenarioContext, protocolComponentsRegistry: ProtocolComponentsRegistry, next: ActorRef) = {
    ...
    system.actorOf(JmsReqOnlyAction.props(attributes, jmsComponents(protocolComponentsRegistry).jmsProtocol, tracker, statsEngine, next), actorName("jmsReqOnly"))
  }
}
