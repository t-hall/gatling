...
import io.gatling.jms.action.{ JmsReqOnlyActionBuilder, JmsReqReplyActionBuilder }
...
case class JmsRequestBuilderBase(requestName: String) {
  ...
  def reqOnly(implicit configuration: GatlingConfiguration) = JmsRequestBuilderQueue(requestName, JmsReqOnlyActionBuilder.apply)
}
