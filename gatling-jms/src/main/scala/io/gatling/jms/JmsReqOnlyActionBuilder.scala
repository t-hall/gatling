...
case class JmsReqOnlyActionBuilder(attributes: JmsAttributes) extends ActionBuilder {
  ...
  def build(system: ActorSystem, next: ActorRef, ctx: ScenarioContext) = {
    ...
    system.actorOf(JmsReqOnlyAction.props(attributes, jmsProtocol, tracker, ctx.dataWriters, next), actorName("jmsReqOnly"))
  }
}
